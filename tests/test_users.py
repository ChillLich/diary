...
def test_custom_err_handlers(client):
    try:
        from core import urls as blogdiary_urls
    except Exception:
        raise AssertionError("Убедитесь, в головном файле с маршрутами нет ошибок.")
    urls_src_squashed = squash_code(inspect.getsource(blogdiary_urls))
...
def get_extra_urls(
    ...
    anothers_page_links = set(find_links_between_lines(base_content, **find_links_kwargs))
    diff_urls = [
        x.get("href")
