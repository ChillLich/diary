...
def test_csrf_failure_view():
    ...
    try:
        ...
        raise AssertionError(
            f"Убедитесь, что view-функция `{csrf_failure_view_setting}`" " работает без ошибок."
        )
    else:
        csrf_status = 403
...
def test_custom_err_handlers(client, user_client):
    ...
    for status, fname in err_pages_vs_file_names.items():
        ...
        assert os.path.isfile(fpath.resolve()), f"Убедитесь, что файл шаблона `{fpath}` существует."

    try:
        from blogdiary.urls import handler500
    except Exception:
        raise AssertionError(
