...
def test_rus_localization():
    ...
    assert settings.LANGUAGE_CODE == "ru-RU", (
        "В настройках приложения значение ключа " "`LANGUAGE_CODE` должно быть `ru-RU`."
    )
...
def test_blog_in_rus():
    ...
    assert blog_app.verbose_name == "Блог", "Приложение `Blog` не локализированно."
...
def test_models_translated(n_model, n_verbose, n_verbose_plural):
    ...
    found_model = [model for model in models if model._meta.object_name == n_model]
    ...
    assert found_model._meta.verbose_name == n_verbose, assert_msg_template.format(
        n_model=n_model, param_name="verbose_name"
    )
    assert found_model._meta.verbose_name_plural == n_verbose_plural, assert_msg_template.format(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Location", "created_at", "Дата создания"),
...
@pytest.mark.parametrize(
    ("n_model", "param", "text"),
    [
        ("Category", "is_published", "Снимите галочку, чтобы скрыть публикацию."),
        (
            "Category",
            "slug",
