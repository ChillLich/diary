...
def get_post_list_context_key(user_client, page_url, page_load_err_msg, key_missing_msg):
    try:
...
class _TestModelAttrs:
    ...
    def test_model_attrs(
        ...
        field_error = field_error or (f"В модели `{model_name}` укажите атрибут `{field}`.")
...
def CommentModel() -> Model:
    ...
    class_defs = re.findall(r"(class +\w+[\w\W]+?)(?=class)", models_src_clean + "class")
    comment_class_name = ""
...
def get_get_response_safely(
    user_client: Client, url: str, err_msg: Optional[str] = None, expected_status=HTTPStatus.OK
) -> HttpResponse:
...
def get_a_post_get_response_safely(user_client: Client, post_id: Union[str, int]) -> HttpResponse:
...
def restore_cleaned_data(cleaned_data: dict) -> dict:
    ...
    cleaned_data_fixed = {k: v.id if isinstance(v, Model) else v for k, v in cleaned_data.items()}
...
def cleanup(request):
    ...
    from blogdiary import settings
